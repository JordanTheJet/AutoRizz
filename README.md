...
AutoRizz is a prosumer fork of CellClaw that adds cloud sync, credit-based usage, and a managed proxy so users don't need their own API keys.

### Subscription Plans

AutoRizz uses monthly subscription plans with credit allocations. **1 credit ≈ $0.01 of AI usage**. No API keys needed; requests route through the AutoRizz proxy.

| Plan | Credits/mo | Price | AI Modes |
|------|-----------|-------|----------|
| Free | 100 | $0 | Fast |
| Starter | 1,000 | $4.99/mo | Fast, Good |
| Pro | 10,000 | $19.99/mo | Fast, Good, Thinking |
| Ultra | 100,000 | $99/mo | Fast, Good, Thinking |

New accounts get **100 free credits** to start. Unused credits roll over 1 month.

#### AI Modes

Users pick an AI mode — AutoRizz picks the best model server-side.

| Mode | Multiplier | Best For |
|------|-----------|----------|
| Fast | 1x | Quick responses, simple tasks |
| Good | 4x | Balanced quality and speed |
| Thinking | 15x | Deep reasoning, complex tasks |

#### Cost Formula

```
credits = ceil((input_tokens × mode_multiplier + output_tokens × 3 × mode_multiplier) / 1000)
```

Output tokens cost 3× input because they're more expensive from providers. Minimum charge is 1 credit per request.

### Multi-Backend Build Flavors

AutoRizz supports swappable backends via Gradle product flavors:

| Flavor | Stack | Cost |
|--------|-------|------|
| `supabase` | Supabase Auth + PostgreSQL + Edge Functions | ~$25/mo |
| `cloudflare` | JWT + D1 + Workers | ~$5/mo |
| `pocketbase` | PocketBase Auth + SQLite + SSE | ~$4/mo |
...
- **Auth** — Email signup with auto-confirm, session management
- **Database** — 10 tables with RLS policies (user profiles, conversations, messages, credits, usage logs, etc.)
- **Edge Functions** — `llm-proxy` (multi-provider LLM routing), `version-check` (app update enforcement)
- **Credit Functions** — Atomic `deduct_credits()` and `add_credits()` with tier multipliers and audit logging
...
The app checks `GET /functions/v1/version-check?v={versionCode}` on launch:
- **Soft prompt** when a newer version exists (`update_available: true`)
- **Force update** when below minimum version (`force_update: true`)

Controlled via the `app_config` table — update `min_version_code` to force users to upgrade.

## Build from Source

```bash
git clone https://github.com/JordanTheJet/AutoRizz.git
cd AutoRizz

# Build debug APK (Supabase flavor)
./gradlew assembleSupabaseDebug
...
```
app/src/main/kotlin/
├── com/cellclaw/          # Upstream CellClaw code
...
│   ├── service/           # Foreground service, Accessibility, Overlay
...
└── com/autorizz/         # AutoRizz additions
    ├── auth/              # AuthManager (delegates to backend AuthService)
    ├── backend/           # Shared interfaces (Auth, Credit, Proxy, Sync, Profile)
    ├── credits/           # CreditManager, CreditCalculator
    ├── di/                # AutoRizzModule (Hilt overrides)
    ├── mode/              # ModeManager, AutoRizzProviderManager
    ├── proxy/             # ProxyProvider (LLM proxy client)
    ├── sync/              # SyncEngine
    └── version/           # VersionCheck (app update enforcement)

app/src/supabase/          # Supabase backend implementations
app/src/cloudflare/        # Cloudflare backend implementations
app/src/pocketbase/        # PocketBase backend implementations

supabase/
├── migrations/            # SQL schema and seed data
└── functions/             # Edge functions (llm-proxy, version-check)
