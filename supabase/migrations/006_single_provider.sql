-- Simplify to single provider (Vertex AI) with 2 modes: fast (1x) and thinking (5x)
-- deduct_credits now accepts an explicit multiplier instead of guessing from model name

-- 1. Update deduct_credits to accept multiplier parameter
...
-- 2. Update subscription plans: all plans get fast+thinking modes
UPDATE subscription_plans SET features = '{"ai_modes": ["fast", "thinking"]}';

-- 3. Default ai_mode to 'fast' for any users who had 'good' selected
UPDATE user_profiles SET ai_mode = 'fast' WHERE ai_mode = 'good';
